...
class BlobAnalysis:
    def __init__(self, thresh):  # Constructor. BW is a binary image in the form of a numpy array
        self.thresh = thresh
        _, contours_list, _ = cv2.findContours(thresh.astype(np.uint8),
                                               cv2.RETR_TREE,
...
def hand_tracker():
    ...
    screenFlipped = pygame.display.set_mode((xSize, ySize),
                                            pygame.RESIZABLE)  # creates surface that will be flipped (mirror display)
    ...
    while not done:
        ...
        for i in range(blobData.counter):  # Iterate from 0 to the number of blobs minus 1
            ...
            centroidList.append(
                blobData.centroid[i])  # Adds the centroid tuple to the centroidList --> used for drawing
        ...
        try:
            ...
            if dummy:
                ...
                cArea = cacheAppendMean(cHullAreaCache,
                                        blobData.cHullArea[0])  # Normalizes (gets rid of noise) in the convex hull area
